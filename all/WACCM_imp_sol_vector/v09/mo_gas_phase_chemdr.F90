...
!
    ...
    REAL(KIND=rkind_comp), INTENT(INOUT) :: delt
...
!
    ...
    REAL(KIND=rkind_comp) :: kgenref_vmr(ncol,pver,gas_pcnst)
...
!    REAL(KIND=rkind_io) :: het_rates_chnks_io(ncol*pver,max(1,gas_pcnst))
    ...
    IF (kgen_istrue) THEN
        ...
        CALL kgen_array_sumcheck("kgenref_vmr", kgen_array_sum, REAL(SUM(kgenref_vmr), kgen_dp), .TRUE.)
    END IF 
