...
!
    ...
    PUBLIC size_dist_param_liq, size_dist_param_basic, rising_factorial, ice_deposition_sublimation, sb2001v2_liq_autoconversion, &
    &sb2001v2_accre_cld_water_rain, kk2000_liq_autoconversion, ice_autoconversion, immersion_freezing, contact_freezing, &
    &snow_self_aggregation, accrete_cloud_water_snow, secondary_ice_production, accrete_rain_snow, heterogeneous_rain_freezing, &
    ...
    &graupel_rain_riming_snow, graupel_rime_splintering, evaporate_sublimate_precip_graupel, avg_diameter_vec 
...
!$acc declare copyin (rv,cpp,tmelt,xxlv,xxls,gamma_bs_plus3,   &
...
SUBROUTINE kr_externs_in_micro_mg_utils(kgen_unit) 
    ...
    CALL kr_micro_mg_utils_mghydrometeorprops(mg_graupel_props, kgen_unit, "mg_graupel_props", .FALSE.) 
    CALL kr_micro_mg_utils_mghydrometeorprops(mg_hail_props, kgen_unit, "mg_hail_props", .FALSE.) 
...
RECURSIVE SUBROUTINE kv_micro_mg_utils_mghydrometeorprops(varname, check_status, var, kgenref_var) 
    ...
    ELSE 
        ...
        IF (rmsdiff_lambda_bounds > kgen_tolerance) THEN 
            dtype_check_status%numOutTol = dtype_check_status%numOutTol + 1 
            IF (kgen_verboseLevel > 1) THEN 
                IF (check_status%rank == 0) THEN 
    ...
    ELSE IF (check_result == CHECK_OUT_TOL) THEN 
        IF (kgen_verboseLevel > 2) THEN 
            IF (check_status%rank ==0) THEN 
                WRITE (*, *) count( var%lambda_bounds /= kgenref_var%lambda_bounds), " of ", size( var%lambda_bounds ), " &
