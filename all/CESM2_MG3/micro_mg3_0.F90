...
!
    ...
    USE wv_sat_methods, ONLY: qsat_water => wv_sat_qsat_water_vect, qsat_ice => wv_sat_qsat_ice_vect 
! Parameters from the utilities module.

    USE micro_mg_utils, ONLY: r8, pi, omsm, qsmall, mincld, rhosn, rhoi, rhow, rhows, bc, ai, bi, aj, bj, ar, br, as, bs, ag, bg, &
    &ah, bh, rhog, rhoh, mi0, rising_factorial 
...
!$acc                 g,r,rv,cpp,tmelt,xxlv,xlf,xxls,microp_uniform,   &
...
PUBLIC kr_externs_out_micro_mg3_0 
...
subroutine micro_mg_tend ( &
  ...
  ! Constituent properties.
    ...
    USE micro_mg_utils, ONLY: mg_liq_props, mg_ice_props, mg_rain_props, mg_graupel_props, mg_hail_props, mg_snow_props 
  ! Size calculation functions.

    USE micro_mg_utils, ONLY: size_dist_param_liq, size_dist_param_basic, avg_diameter_vec 
  ! Microphysical processes.
    ...
    &graupel_collecting_snow, graupel_collecting_rain, graupel_collecting_cld_water, graupel_riming_liquid_snow, &
    &graupel_rain_riming_snow, graupel_rime_splintering, evaporate_sublimate_precip_graupel 
  ...
  mdust = size(rndst,3)
  !$acc data present (mg_liq_props,mg_ice_props,mg_rain_props,            &
  !$acc               mg_graupel_props,mg_hail_props,mg_snow_props,       &
  !$acc               t,q,qcn,qin,ncn,nin,qrn,qsn,nrn,nsn,qgr,ngr,relvar, &
  !$acc               accre_enhan,p,pdel,cldn,liqcldf,icecldf,qsatfac,    &
  !$acc               naai,npccn,rndst,nacon,tnd_qsnow,tnd_nsnow,re_ice,  &
  !$acc               frzimm,frzcnt,frzdep,                               &
  !$acc               qcsinksum_rate1ord,tlat,qvlat,qctend,qitend,nctend, &
  ...
  !$acc               qric,qgic,ncic,niic,nsic,nric,ngic,nimax,lami,n0i,  &
  ...
  do k=1,nlev
     do i=1,mgncol
        ...
        if (do_cldice .and. nitend(i,k).gt.0._r8.and.ni(i,k)+nitend(i,k)*deltat.gt.nimax(i,k)) then
           nitend(i,k)=max(0._r8,(nimax(i,k)-ni(i,k))*rdeltat)
        end if
     end do
  end do ! end k loop
...
subroutine calc_rercld(lamr, n0r, lamc, pgam, qric, qcic, ncic, rercld, vlen)
  ...
  !$acc parallel vector_length(VLENS) default(present)
  !$acc loop gang vector private(Atmp)
...
!read state subroutine for kr_externs_in_micro_mg3_0 
SUBROUTINE kr_externs_in_micro_mg3_0(kgen_unit) 
    ...
    READ (UNIT = kgen_unit) ngcons 
    READ (UNIT = kgen_unit) nrcons 
    READ (UNIT = kgen_unit) nscons 
    READ (UNIT = kgen_unit) ncnst 
    READ (UNIT = kgen_unit) ninst 
    READ (UNIT = kgen_unit) ngnst 
    READ (UNIT = kgen_unit) nrnst 
    READ (UNIT = kgen_unit) nsnst 
    READ (UNIT = kgen_unit) icenuc_rh_off 
    READ (UNIT = kgen_unit) icenuc_use_meyers 
    READ (UNIT = kgen_unit) evap_scl_ifs 
    READ (UNIT = kgen_unit) evap_rhthrsh_ifs 
    READ (UNIT = kgen_unit) ifs_sed 
    READ (UNIT = kgen_unit) precip_fall_corr 
    ...
    READ (UNIT = kgen_unit) do_hail 
    READ (UNIT = kgen_unit) do_graupel 
    ...
    READ (UNIT = kgen_unit) gamma_bg_plus1 
    READ (UNIT = kgen_unit) gamma_bg_plus4 
...
!read state subroutine for kr_externs_out_micro_mg3_0 
SUBROUTINE kr_externs_out_micro_mg3_0(kgen_unit) 
    INTEGER, INTENT(IN) :: kgen_unit 
      
    LOGICAL :: kgen_istrue 
    REAL(KIND=8) :: kgen_array_sum 
END SUBROUTINE kr_externs_out_micro_mg3_0 
